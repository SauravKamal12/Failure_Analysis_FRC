%Code for finding ply failure loads using partial degradation
...
%CTE & CME vectors in global axes
for i = 1:n
    th = theta(i);
    c = cosd(th);
    s = sind(th);
...
Output = fopen('Partial Degradation.txt', 'w');
...
while (size(DL,1) < n)
    ...
    %Finding stresses considering only mechanical loading
    ...
    %Findling which plies will fail and in what mode
    for j = 1:n
